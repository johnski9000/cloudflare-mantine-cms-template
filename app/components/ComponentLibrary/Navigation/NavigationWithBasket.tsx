...
import { HiShoppingBag, HiMenu, HiPhone, HiMail } from "react-icons/hi";
...
export const NavigationWithBasketProps = {
  id: igniteString("navigation-with-basket"),
  ...
  cta: igniteString("Book Now"),
  ctaUrl: igniteString("/store"),
  ecommerce: igniteBoolean(true),
  fadeOnScroll: igniteBoolean(false),
};

export default function NavigationWithBasket(props) {
  ...
  const isEcommerceEnabled = mergedProps.ecommerce.value;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex md:items-center md:space-x-8 flex-1 justify-center">
              <Group spacing="xl" ref={dropdownRef}>
            ...
            {/* Right side - CTA + Basket + Mobile Menu */}
            <div className="flex items-center space-x-3">
              {/* Basket Icon - Only render if ecommerce is enabled */}
              {isEcommerceEnabled && <BasketIcon />}

              {/* Desktop CTA Button */}
              <Link href={mergedProps.ctaUrl.value} className="hidden md:block">
                <Button
                  variant="gradient"
                  radius="xl"
                  size="sm"
              ...
              <ActionIcon
                size="lg"
                variant="subtle"
                className={`md:hidden transition-colors duration-300 text-gray-700 hover:text-indigo-600`}
                onClick={open}
              >
                <HiMenu size={24} />
              </ActionIcon>
